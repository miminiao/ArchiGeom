...
from lib.geom import Node,Edge,Arc,Loop,Poly
...
def parse_data(j_data:list,center_to_zero:bool=True)->tuple:
    ...
    nodes:list[Node]=[]
    wall_lines:list[Edge]=[]
    watch_lines:list[Edge]=[]
    opening_lines:list[Edge]=[]
    window_blocks=[]
    door_blocks=[]
    block_defs={}
    trans=None  # 归零向量
    for ent in j_data:
        match ent["object_name"]:
            ...
            case "line":
                ...
                s.add_edge_in_order(Edge(s,e))
                e.add_edge_in_order(Edge(e,s))
                new_edge=Edge(s,e)
            ...
            case "arc":
                ...
                if category is not None: 
                    # category.append(new_edge)  # TODO
                    category.append(Edge(new_edge.s,new_edge.e))  # 先按直线段处理
            case "polyline":
                ...
                for i in range(seg_num):
                    ...
                    s.add_edge_in_order(Edge(s,e))
                    e.add_edge_in_order(Edge(e,s))
                    new_edge=Edge(s,e) if abs(bulge)<const.TOL_VAL else Arc(s,e,bulge)
                    category=map_layer(ent["layer"])
                    if category is not None: category.append(new_edge)
            case _:continue
    return wall_lines,watch_lines,opening_lines,window_blocks,door_blocks,block_defs
def group_lines_by_angle(lines:list[Edge])->tuple[list[list[Edge]],list[Vec3d],list[Vec3d]]:
    """将平行的墙线分成一组，并按法向量排序
...
def get_unit_and_normal_of_parallel_lines(lines:list[Edge])->tuple[Vec3d,Vec3d]:
    """返回一组平行线的单位向量和法向量"""
    # 找一根最长的，作为方向向量
...
def find_walls_from_parallel_lines(line_group:list[Edge],wall_width_limit:float)->list[Wall]:
...
def get_walls_from_domain(line:Wall)->list[Wall]:
    ...
    for sub_dom in line.upper_domain.items:
        ...
        new_baseline=Edge(Node(s_vector.x,s_vector.y),Node(e_vector.x,e_vector.y)).offset(sub_dom.value/2)
    ...
    return walls
def remove_watch_window_and_railing(watch_lines:list[Edge],
...
def find_butress(wall_outline_loops:list[Loop])->set[Edge]:
    """根据墙线轮廓，找到墙垛"""
    butresses=set()
    for outline in wall_outline_loops:
        ...
        for i,edge in enumerate(outline.edges):
            ...
            if (abs(pre_edge.to_vec3d().unit().cross(edge.to_vec3d().unit()).z-1)<const.TOL_DIST2 
                and abs(edge.to_vec3d().unit().cross(succ_edge.to_vec3d().unit()).z-1)<const.TOL_DIST2): # 2次90度左转
                butresses.add(edge)
        ...
        for i,edge in enumerate(outline.edges):
            ...
            if edge in butresses and pre_edge in butresses and succ_edge in butresses: # 夹在中间
                if len(outline.edges)==4 and (edge.length<pre_edge.length or edge.length<succ_edge.length): # 连续4个的情况，去除长的
                    continue
    ...
    return butresses
def find_opening_regions(butresses:set[Edge],
    ...
    Args:
        butresses (set[Edge]): _description_
    ...
    """
    def match_butress_lines(dist_condition:typing.Callable[[Edge,Edge],bool]):
        for i,line in enumerate(lines):
            ...
            for j in others_idx:
                ...
                if (domains[line].is_overlap(domains[other], include_endpoints=False)  # 有重叠部分
                    ...
                    break  # 只找最近的一个判断
        return
    def is_pair_butress(line:Edge,other:Edge)->bool:
        """两个相对的垛子"""
    ...
    def is_single_butress(line:Edge,other:Edge)->bool:
        """一个垛子和一个墙"""
    ...
    # 记录每个墙垛的相邻墙垛
    neighbor_butress:dict[Edge,list[Edge]]={}
...
def recognize_windows_and_doors(opening_regions:list[Loop],
    ...
    for region in opening_regions:
        neighbor_lines:list[Edge]=rt_watch_opening_lines.query(region.get_mbb())
        for line in neighbor_lines:
            ...
            for region_bound in region.edges:
                if mid.is_on_edge(region_bound): 
                    break
            else:
    ...
    for opening_block in doors+windows:
        ...
        nearest_region=None
        for region in opening_regions:
            centroid,_=Edge(region.nodes[0],region.nodes[2]).point_at(t=0.5)
...
def _draw_edge(line:Edge,base_color:str="m",edge_color:str="b",alpha:float=0.5):
...
if __name__=="__main__":
    ...
    with open("test/find_wall/case_0_door.json",'r',encoding="utf8") as f:
        j_data=json.load(f)
